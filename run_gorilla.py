...
def wait_for_results(session: requests.Session, start_url: str, poll_s: float, timeout_s: float) -> requests.Response:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("output"),
        help="Directory containing .rnk files (default: output/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("output") / "gorilla_bp",
        help="Where to write PNGs (default: output/gorilla_bp/)",
    )
    parser.add_argument(
        "--species",
    ...
    parser.add_argument(
        "--pvalue",
        default="0.001",
        choices=["0.001", "0.0001", "0.00001", "0.000001", "0.0000001", "0.00000001", "0.000000001", "0.0000000001", "0.00000000001"],
    ...
    session = requests.Session()
    session.headers.update({"User-Agent": "phospho_mhg_anno/run_gorilla.py (requests)"})

    print(f"Input: {input_dir} ({len(rnk_files)} .rnk files)")
    print(f"Output PNGs: {out_dir}")
    print(f"Species: {args.species} ({species_value}), ontology: Biological process (proc), fast_mode={fast_mode}")
